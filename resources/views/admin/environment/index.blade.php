...
@section('environment') active @endSection
...
@section('content')
 <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">ویرایش صفحه محیط</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-left">
              <li class="breadcrumb-item"><a href="{{url('admin/dashboard')}}">خانه</a></li>
              <li class="breadcrumb-item active">صفحه محیط</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="row" style="padding: 25px">
            <form style="width: 100%" action="{{url('admin/environment')}}" method="POST" enctype="multipart/form-data">
                @csrf
              <div class="form-group">
                 <label for="title">عنوان انگلیسی</label>
                 <input class="form-control" style="direction: ltr" required type="text" id="title" name="title[en]" value="{{$environment['title']->value}}">
              </div>
              <div class="form-group">
                 <label for="title_fa">عنوان فارسی</label>
                 <input class="form-control" required type="text" id="title_fa" name="title[fa]" value="{{$environment['title']->value_fa}}">
              </div>
              @can('arabic')
              <div class="form-group">
                 <label for="title_ar">عنوان عربی</label>
                 <input class="form-control" required type="text" id="title_ar" name="title[ar]" value="{{$environment['title']->value_ar}}">
              </div>
              @endcan
              <div class="form-group" style="text-align:left">
                 <label for="text">متن انگلیسی</label>
                 <textarea class="form-control" required type="text"id="text" name="text[en]" rows="8">{!!$environment['text']->value!!}</textarea>
              </div>
              <div class="form-group">
                <label for="text_fa">متن فارسی</label>
                 <textarea class="form-control" required type="text" id="text_fa" name="text[fa]" rows="8">{!!$environment['text']->value_fa!!}</textarea>
              </div>
               @can('arabic')
              <div class="form-group">
                <label for="text_ar">متن عربی</label>
                 <textarea class="form-control" required type="text" id="text_ar" name="text[ar]" rows="8">{!!$environment['text']->value_ar!!}</textarea>
              </div>
              @endcan
              <div class="form-group">
                <label for="image">تصویر اول</label>
                <br>
                <img id="image-one" src="{{asset($environment['image']->value ?? '')}}" width="400" alt="">
                <br>
                <br>
              ...
              <div class="form-group">
                ...
                <br>
                <img id="image-two" src="{{asset($environment['image1']->value ?? '')}}" width="400" alt="">
                <br>
                <br>
